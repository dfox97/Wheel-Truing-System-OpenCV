	...
	for (int i = 0; i < width; i++)//sames loop as above , sets the pixel line to track the location 
	{
		line(histImage, Point(bin_w*(i), hist_h), Point(bin_w*(i), hist_h - hist[i]), Scalar(0, 0, 0), 1, 8, 0);//draws the intensity line to track location
               /*points to the width and height values which creates the line ,bin_w(final value from the cvround operation with value of i),
				, hist_h - hist[i] takes away original height with width, allows to make vertical line to track the middle of the screen
...
int main(int, char**)
{
	...
	int filtintensity[640];// sets intensity values and filters makes the lines more clear 
	int left_location;// location values if the wheel moves left  , used to work out the final position
    int right_location;// location values if the wheel moves left , used to work out the final position
	int x_co;//stores x and y values
	...
	for (;;)// for loop same as using while
	{
		...
		{
			intensity[x] = (int)grey.at<uchar>(y, x); //original intensity, (int) allows the value from the uchar(unsigned character) to be turned into a int.
		}
		...
		for (int i = 230; i < (grey.cols - 10); i++)  //iteration for the start point of where it is determined across the column lines (allow 10mm from the left boundary of the wheel edge)
													  //230 can be changed depending if we want the histogram to start tracking at a certain height , 120 is a good value for the bike wheel 
		{
			...
			{
				left_location = i;   //initialise left location to the new value of i
				break;  //terminates loop
			}
		}
        
		for (int j = 410; j < (grey.cols - 10); j--)  //iteration for the start point of where it is determined across the column lines (allow 10mm from the left boundary of the wheel edge)
							  //number (410) can be changed depending if we want the histogram to start tracking at a certain height
		{
			if (filtintensity[j + 10] >(filtintensity[j] + 10))//condition for if the pixel is 10 pixels above the iterative line being scanned 
																   //if the filterintensity is 10pixels above the normal threshold then it will start track the location
			{
                right_location = j;   //initialise right location to the new value of i
				break;  //terminates loop
			}
		}
		...
		x_co = (((left_location + right_location)/2)-320) / 10;//calculated the x value by taking the locations dividing by 2 and then minusing 320 assume the pixel value is 10 pixel = 1mm

		printf("X:%d\n", x_co); //prints out the final value
